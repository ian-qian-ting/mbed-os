...
set SECTIONS_RAM=-j .image2.table -j .text -j .data
...
::%tooldir%\..\..\prepend_header.sh %bindir%%IMAGE2%.bin %IMAGE2_START_SYMBOL% %bindir%%BINROOT%-%POSTFIX%.map
::%tooldir%\..\..\prepend_header.sh %bindir%%IMAGE3%.bin %IMAGE3_START_SYMBOL% %bindir%%BINROOT%-%POSTFIX%.map
